...
function formatTime(value, unit, precision) {
    let val;
    let suffix;
    if (unit === 's') { val = value; suffix = ' s'; }
    else if (unit === 'ms') { val = value * 1000; suffix = ' ms'; }
    else { val = value * 1000000; suffix = ' μs'; }
...
function determineTimeUnit(visibleSpan) {
    if (visibleSpan >= 0.1) {
        return { unit: 's', precision: Math.max(3, getDecimalPlaces(visibleSpan / 10)) };
    }
    if (visibleSpan >= 0.0001) {
        return { unit: 'ms', precision: Math.max(3, getDecimalPlaces(visibleSpan * 1000 / 10)) };
    }
    return { unit: 'us', precision: Math.max(3, getDecimalPlaces(visibleSpan * 1000000 / 10)) };
}
...
function processData(results) {
    try {
        ...
        for(let i=0; i<len; i++) {
            ...
            if (typeof t === 'number' && typeof c === 'number' && !isNaN(t) && !isNaN(c)) {
        ...
        showToast(`Chart loaded with ${AppState.data.length.toLocaleString('en-US')} points.`, 3000);
...
function calculateGlobalStats() {
    ...
    if(AppState.globalStats) {
        AppState.globalStats.count = AppState.globalStats.count.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".");
    }
    updateGlobalStatsUI();
}
...
function updateDisplay(data, map) {
    for (const item of map) {
        ...
        if (element) {
            const value = item.formatter ? item.formatter(data) : (data ? data[item.key] : '---');
...
const globalStatsMap = [
    { id: 'stats-count', key: 'count', formatter: d => d ? d.count.toLocaleString('en-US') : '---' },
    { id: 'stats-min', key: 'minVal', formatter: d => d ? d.minVal.toFixed(2) + ' mA' : '---' },
    { id: 'stats-max', key: 'maxVal', formatter: d => d ? d.maxVal.toFixed(2) + ' mA' : '---' },
    { id: 'stats-avg', key: 'avgVal', formatter: d => d ? d.avgVal.toFixed(2) + ' mA' : '---' },
];

// Map for digital display with analysis
const digitalDisplayMap = [
    { id: 'display-dt', key: 'finalTimeDeltaFormatted', formatter: d => d ? d.finalTimeDeltaFormatted : '---' },
    { id: 'display-count-ab', key: 'count', formatter: d => d ? d.count.toLocaleString('en-US') : '---' },
    { id: 'display-max', key: 'maxVal', formatter: d => d ? d.maxVal.toFixed(2) + ' mA' : '---' },
    { id: 'display-avg', key: 'avgVal', formatter: d => d ? d.avgVal.toFixed(2) + ' mA' : '---' },
];
...
const baseChartOption = {
    ...
    tooltip: {
        ...
        formatter: function (params) {
            ...
            const isAnalysisComplete = AppState.cursorA && AppState.cursorB && analysis;
            ...
            if (isAnalysisComplete) {
                ...
                html += '<span style="color: #007bff; font-weight: bold;">Measurement Result (A - B)</span><br/>';
            ...
            if (AppState.cursorA && !AppState.cursorB) {
                html += '<hr style="margin: 4px 0;">';
                html += '<span style="color: #DC3545; font-weight: bold;">Waiting for Cursor B...</span>';
            }
            return html;
        }
    },
    legend: { data: ['Current'] },
    grid: [{ left: '3%', right: '4%', bottom: '15%', containLabel: true, z: 2, id: 'mainGrid' }],
    xAxis: [{
        gridIndex: 0, id: 'mainXAxis', type: 'value', name: 'Time',
        min: 'dataMin', max: 'dataMax', nameLocation: 'middle',
        nameTextStyle: { padding: [20, 0, 0, 0] },
        axisLabel: { formatter: value => value.toFixed(AppState.xAxisPrecision) + ' s' }
    }],
    yAxis: {
        gridIndex: 0, type: 'value', name: 'Current (mA)',
    ...
    dataZoom: [
        ...
        {
            type: 'slider', xAxisIndex: [0], bottom: 0, height: 40,
            showDataShadow: true, fillerColor: 'rgba(255,165,0,0.2)',
            dataBackground: { areaStyle: { color: DATA_SHADOW_COLOR } },
            handleStyle: { color: '#ccc', shadowBlur: 3, shadowColor: 'rgba(0, 0, 0, 0.6)', shadowOffsetX: 2, shadowOffsetY: 2 },
            realtime: false, 
            throttle: 0,
        },
    ],
    series: [
        {
            name: 'Current', type: 'line', xAxisIndex: 0, smooth: false,
            sampling: 'lttb',
            showSymbol: AppState.isSymbolsVisible,
            data: [],
            lineStyle: { width: 1, color: '#4a90e2' }, itemStyle: { color: '#007bff' },
            id: 'mainSeries', markLine: { symbol: 'none', data: [] }, markArea: { data: [] }
        },
        {
            name: 'Cursors', type: 'scatter', symbolSize: 10, data: [],
            itemStyle: { color: 'rgba(0, 0, 0, 0)', borderColor: '#e74c3c', borderWidth: 2, opacity: 1 },
...
function renderChart() {
    try {
        ...
        if (!AppState.chartInstance) {
            ...
            AppState.chartInstance = echarts.init(CHART_DOM, null, { renderer: 'svg' });
        ...
        // Apply scale
        baseChartOption.yAxis.min = AppState.yScale.min === null ? 'dataMin' : AppState.yScale.min;
        baseChartOption.yAxis.max = AppState.yScale.max === null ? 'dataMax' : AppState.yScale.max;
    ...
    setTimeout(() => {
        updateChartOption({
            dataZoom: [
                { type: 'inside', xAxisIndex: [0] },
                {
                    dataBackground: { areaStyle: { color: DATA_SHADOW_COLOR, opacity: 1 } },
                    showDataShadow: true,
                    type: 'slider',
                    xAxisIndex: [0]
                }
...
function updateXAxisPrecision() {
    ...
    const dataZoomOpt = AppState.chartInstance.getOption().dataZoom.find(dz => dz.xAxisIndex && dz.xAxisIndex.includes(0));
    ...
    const startValue = dataStart + (dataEnd - dataStart) * (startPercentage / 100);
    const endValue = dataStart + (dataEnd - dataStart) * (endPercentage / 100);
    const visibleSpan = Math.abs(endValue - startValue);

    const { unit: newUnit, precision: newPrecision } = determineTimeUnit(visibleSpan);

    if (newPrecision !== AppState.xAxisPrecision || newUnit !== AppState.timeUnit) {
        ...
        updateChartOption({
            xAxis: [{
                id: 'mainXAxis',
                name: `Time (${AppState.timeUnit === 'us' ? 'μs' : AppState.timeUnit})`,
                axisLabel: { formatter: value => formatTime(value, AppState.timeUnit, AppState.xAxisPrecision) }
            }]
        });
    }
}
...
function binarySearchClosest(arr, target) {
    ...
    return (Math.abs(valLeft - target) < Math.abs(valPrev - target)) ? left : left - 1;
}
...
function handleChartClick(params) {
    ...
    const pointInGrid = AppState.chartInstance.convertFromPixel('grid', pointInPixel);
    ...
    if (AppState.cursorA === null || AppState.cursorB !== null) {
        ...
        showToast(`Cursor A set at ${formatTime(time, AppState.timeUnit, AppState.xAxisPrecision)} (${current.toFixed(2)} mA)`, 1000);
    } else {
        AppState.cursorB = { time: time, current: current, name: 'B' };
        calculateAnalysis();
        showToast(`Cursor B set at ${formatTime(time, AppState.timeUnit, AppState.xAxisPrecision)} (${current.toFixed(2)} mA)`, 1000);
    }
...
function calculateAnalysis() {
    if (!AppState.cursorA || !AppState.cursorB) { AppState.analysisResults = null; return; }
    ...
    const finalTimeDelta = AppState.timeIndex[endIdx] - AppState.timeIndex[startIdx];
    const { unit: deltaUnit, precision: deltaPrecision } = determineTimeUnit(finalTimeDelta);
    const finalTimeDeltaFormatted = formatTime(finalTimeDelta, deltaUnit, Math.max(deltaPrecision, 6));
...
function updateCursorVisuals() {
    ...
    if (AppState.cursorA && AppState.cursorB) {
        ...
        markAreas.push([
            { xAxis: AppState.cursorA.time, xAxisIndex: 0, itemStyle: { color: MARK_AREA_COLOR, opacity: 0.5 } },
...
function exportChartAsSVG() {
    ...
    try {
        const globalData = AppState.globalStats ?
            `Points: ${AppState.globalStats.count.toLocaleString('en-US')} | Overall Avg: ${AppState.globalStats.avgVal.toFixed(2)} mA | Overall Max: ${AppState.globalStats.maxVal.toFixed(2)} mA` :
            'Global Statistics: N/A';

        const analysisData = AppState.analysisResults ?
            `Analysis (A-B): Δt: ${AppState.analysisResults.finalTimeDeltaFormatted} | Points: ${AppState.analysisResults.count.toLocaleString('en-US')} | Avg: ${AppState.analysisResults.avgVal.toFixed(2)} mA | Max: ${AppState.analysisResults.maxVal.toFixed(2)} mA` :
            'Analysis (A-B): Cursors A and B not defined';

        const svgElement = CHART_DOM.querySelector('svg');
        if (!svgElement) { showToast("Error: Chart not rendered in SVG.", 2000); return; }
        ...
        g.appendChild(createText(yPos, '14', 'bold', '#333', "Analysis Information:"));
        yPos += lineHeight;
        g.appendChild(createText(yPos, '12', 'normal', '#555', `Global: ${globalData}`));
        yPos += lineHeight;
        ...
        link.download = `osciloscopio_export_${new Date().toISOString().slice(0, 10)}.svg`;
        link.href = url;
...
function renderCursorOverlays() {
    ...
    if (!AppState.cursorA && !AppState.cursorB) {
        const existing = CHART_DOM.querySelectorAll('.chart-cursor-indicator, .cursor-label');
        existing.forEach(n => n.remove());
        return;
    }

    const sampleY = (AppState.ampIndex && AppState.ampIndex.length) ? AppState.ampIndex[Math.floor(AppState.ampIndex.length / 2)] : 0;

    let gridTop = 0, gridHeight = CHART_DOM.clientHeight;
    try {
    ...
    if (AppState.cursorA && typeof AppState.cursorA.time === 'number') {
        const x = AppState.chartInstance.convertToPixel('grid', [AppState.cursorA.time, sampleY])[0];
        ...
        if (!lab) { lab = document.createElement('div'); lab.className = 'cursor-label label-a'; lab.textContent = 'A'; CHART_DOM.appendChild(lab); }
        lab.style.position = 'absolute'; lab.style.left = `${x}px`; lab.style.top = `${Math.max(0, gridTop - 18)}px`;
    }

    if (AppState.cursorB && typeof AppState.cursorB.time === 'number') {
        const x = AppState.chartInstance.convertToPixel('grid', [AppState.cursorB.time, sampleY])[0];
        ...
        if (!lab) { lab = document.createElement('div'); lab.className = 'cursor-label label-b'; lab.textContent = 'B'; CHART_DOM.appendChild(lab); }
        lab.style.position = 'absolute'; lab.style.left = `${x}px`; lab.style.top = `${Math.max(0, gridTop - 18)}px`;
    }
}
